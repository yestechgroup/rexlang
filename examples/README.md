...
The five canonical examples double as the repo's capability harness: the test
suites under `crates/rex-backend-rust/tests/examples_conformance.rs`,
`crates/rex-backend-jsonschema/tests/examples_schemas.rs`, and
...
never serialized). For `library`, `ecommerce`, and `org` the documents are
